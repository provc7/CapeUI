        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.switchMainTab = function (tab) {
                ...
                if (mapControls) mapControls.style.display = (tab === 'map' || tab === '3d') ? 'flex' : 'none';
            ...
            function changeMapZoom(factor) {
                ...
                } else if ((currentView === '3d' || currentView === 'signatures') && controls && camera) {
                    const zoomVector = new THREE.Vector3().subVectors(camera.position, controls.target);
                    // factor > 1 means Zoom In, factor < 1 means Zoom Out
                    const scale = factor > 1 ? 0.8 : 1.25;
                    zoomVector.multiplyScalar(scale);
